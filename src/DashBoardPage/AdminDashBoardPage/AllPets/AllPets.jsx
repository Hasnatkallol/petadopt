...
const AllPets = () => {
  ...
  const handleDelete = async (petId) => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axiosSecure.delete(`/adoptPet/${petId}?email=${user.email}`);
  ...
  const handleAdopted = async (petId) => {
    try {
      ...
      if (res.data.modifiedCount > 0) {
        setPets((prevPets) =>
          prevPets.map((pet) =>
            pet._id === petId ? { ...pet, isAdopted: true, adoptionStatus: "Adopted" } : pet
          )
        );
  ...
  const handleAdoptFalse = async (petId) => {
    try {
      ...
      if (res.data.modifiedCount > 0) {
        setPets((prevPets) =>
          prevPets.map((pet) =>
            pet._id === petId ? { ...pet, isAdopted: false, adoptionStatus: "Not Adopted" } : pet
          )
        );
        Swal.fire("Success!", "Pet marked as Not Adopted.", "success");
      }
    } catch (error) {
      console.log(error);
      Swal.fire("Error", "Failed to update adoption status.", "error");
  ...
  return (
    <div className={`p-4 md:p-6 max-w-[95vw] mx-auto ${currentTheme.bg} ${currentTheme.text} min-h-screen`}>
      <h2 className={`text-2xl md:text-3xl font-extrabold mb-6 md:mb-8 text-center ${
      ...
      <div className={`overflow-x-auto shadow-lg rounded-lg border ${currentTheme.card}`}>
        <table className="min-w-full divide-y divide-gray-200">
          <thead className={theme === "light" ? "bg-indigo-100" : "bg-gray-700"}>
            <tr>
              <th className="px-3 py-3 text-left text-xs md:text-sm font-semibold uppercase tracking-wider">Image</th>
              <th className="px-3 py-3 text-left text-xs md:text-sm font-semibold uppercase tracking-wider">Name</th>
              <th className="px-3 py-3 text-left text-xs md:text-sm font-semibold uppercase tracking-wider">Age</th>
              <th className="px-3 py-3 hidden sm:table-cell">Breed</th>
              <th className="px-3 py-3 hidden md:table-cell">Category</th>
              <th className="px-3 py-3 hidden md:table-cell">Gender</th>
              <th className="px-3 py-3 hidden lg:table-cell">Location</th>
              <th className="px-3 py-3">Vaccinated</th>
              <th className="px-3 py-3">Status</th>
              <th className="px-3 py-3 hidden lg:table-cell">Added By</th>
          ...
          <tbody className={theme === "light" ? "divide-y divide-gray-200" : "divide-y divide-gray-700"}>
            {pets.map((pet) => (
              <tr key={pet._id} className={theme === "light" ? "hover:bg-indigo-50" : "hover:bg-gray-700"}>
                <td className="px-3 py-4">
                  <img src={pet.image} alt={pet.name} className="w-12 h-12 md:w-16 md:h-16 object-cover rounded-lg border shadow-sm" />
                </td>
                <td className="px-3 py-4 font-semibold">{pet.name}</td>
                <td className="px-3 py-4">{pet.age}</td>
                <td className="px-3 py-4 hidden sm:table-cell">{pet.breed}</td>
                <td className="px-3 py-4 hidden md:table-cell">{pet.category}</td>
                <td className="px-3 py-4 hidden md:table-cell">{pet.gender}</td>
                <td className="px-3 py-4 hidden lg:table-cell">{pet.location}</td>
                <td className="px-3 py-4 text-center">
                  {pet.vaccinated ? (
                    ...
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                      theme === "light" ? "bg-red-100 text-red-700" : "bg-red-900 text-red-300"
                    }`}>
                      No
                    </span>
                  )}
                  ...
                  {isAdmin ? (
                    <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-2">
                      <button
                        ...
                        className={`px-2 py-1 text-xs md:text-sm rounded-full border font-semibold transition-colors duration-300 ${
                          pet.isAdopted === false
                            ? theme === "light"
                    ...
                    <span className={`px-2 py-1 text-xs md:text-sm rounded-full font-semibold ${
                      pet.isAdopted
                        ? theme === "light"
                ...
                <td className="px-3 py-4 hidden lg:table-cell">{pet.addedBy}</td>
